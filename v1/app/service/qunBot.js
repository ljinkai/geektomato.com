...
const leancloud = require('../service/qunBotLean.js')
...
const puppet = null; //new PuppetPadlocal({ token: "puppet_padlocal_51574bc9a79e4b96ad31d48f1e9bfcc5"})
function initBot() {
    bot = null;
    // new Wechaty({
    //     name: 'padlocal',
    //     puppet,
    //     defaultLoginPolicy: 1
    // })

    bot
        .on('scan', (qrcode, status) => {
            // qrCodeData.status = status
            // console.log("=scan 1=", qrcode, status)
...
class QunBotService extends Service {
    ...
    async stop() {
        ...
        await bot.reset()
        await resetInit()
        // log.info('Bot stop message to pop quit:', 'qhy::stop')
