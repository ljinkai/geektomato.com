...
var fm = {
	_browser: {
		...
		language: EASY_ENV_IS_BROWSER ? (navigator.browserLanguage || navigator.language).toLowerCase() : ''
	},
	...
	_getViewName: function () {
		var referer = document.body.getAttribute('view-name');
		if (!referer) {
			//pageview overhide the view-name
			referer = document.body.getAttribute('pageview');
		}
		return referer || '';
	},
	...
	openItem: function (item) {
		var version = document.getElementById('#serviceListContainer').getAttribute('version');
	...
	getWinHeight: function () {
		...
		if (EASY_ENV_IS_BROWSER) {
			if (navigator.platform == 'iPhone' || navigator.platform == 'iPad' || navigator.platform == 'iPod') {
				// Height + 60px
				return window.document.documentElement.clientHeight + 60;
			}
...
var fmt = {
	send: function (source, action, target, attrMap) {
		var info = document.getElementsByClassName('.dataInfo')[0].getAttribute('info');
	...
	getUserAgentValue: function (key) {
		if (key == 'app_version') {
			...
			} else if (document.body.getAttribute('user')) {
				var user = document.body.getAttribute('user');
	...
	openHelpCenterUrl: function (obj) {
		...
		fm.send('getUser', {}, function (res) {
			if (res.state == 0) {
				...
				var origin =
					'https://www.geektomato.com/mobile/helpCenterRedirect/?redirect=' +
	...
	imagesFilter: function (str, size, mode) {
		...
		if (str) {
			...
			str = str.replace(/http:/, 'https:').replace(/res.geektomatoapp.com/, 'fivemiles-res.cloudinary.com');
			return str;
		}
		return str;
